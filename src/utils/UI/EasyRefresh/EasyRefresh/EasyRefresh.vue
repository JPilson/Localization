<template>
  <div class="v-easy-refresh">
    ...
    <div class="v-easy-refresh-header" :style="'bottom: ' + headerBottom + 'px;'">
      <slot name="header">
        <ClassicsHeader ref="header" :on-refreshing-listener="onRefreshingListener"/>
...
export default class EasyRefresh extends Vue {
  ...
  @Prop({default:false})
  private attachFooterLoader!:boolean
  @Prop() // 刷新回调
  ...
  private mounted() {
    ...
    for (const node of this.$children) {
      if (Object.getOwnPropertyDescriptor(node, 'refreshHeight') && !this.header) {
        this.setHeader((node as unknown) as Header)
      } else if (Object.getOwnPropertyDescriptor(node, 'loadHeight') && !this.footer) {
        this.setFooter((node as unknown) as Footer)
      } else if (Object.getOwnPropertyDescriptor(node, 'updateScrollBar') && !this.scrollBar) {
  ...
  @Watch('loadFinished')
  private onLoadFinished(newFlag:boolean,oldFlag:boolean){
        this.done(newFlag)
  }
