...
<!--            <CircularProgress v-if="headerStatus === 4" :color="textColor" class="er-classics-header-progress"></CircularProgress>-->
            <Done v-if="headerStatus === 5 || headerStatus === 6" :color="textColor" class="er-classics-header-done-icon"></Done>
...
export default class ClassicsHeader extends Vue implements Header {
  ...
  @Prop() // 刷新回调
  private onRefreshingListener!: () => void
    // 提示刷新文字
    @Prop({default: 'Puxe para atualizar'}) // Pull to refresh
    private refreshText!: string
    // 准备刷新文字
    @Prop({default: 'Solte para atualizar'}) // Release to refresh
    ...
    public onRefreshing(): void {
        ...
        this.onRefreshingListener() // let user know that refreshing has started
